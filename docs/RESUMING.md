...
## 📐 설계 완료·구현 대기: 카드 감사 정리 + 커스텀 항목 추가 (2026-05-24)

**이 세션(설계) → 다른 세션(구현)** 으로 핸드오프. 아래 4개 문서가 산출물. 새 세션에서 프로젝트 디렉토리 열고 해당 plan을 `superpowers:executing-plans`(또는 `subagent-driven-development`)로 실행.

| 문서 | 내용 |
|---|---|
| `docs/superpowers/specs/2026-05-24-card-audit-cleanup-design.md` | 카드 정리 설계 |
| `docs/superpowers/plans/2026-05-24-card-audit-cleanup.md` | 6 Task 구현 플랜 |
| `docs/superpowers/specs/2026-05-24-custom-itinerary-item-design.md` | 커스텀 항목 추가 설계(오프라인 재생 모델) |
| `docs/superpowers/plans/2026-05-24-custom-itinerary-item.md` | 7 Task 구현 플랜 |

- **카드 정리 핵심**: 정적 103→94. 제거 9장(`7.23 5.33 5.34 4.23 4.22 1.14 1.19 3.17 3.8`) — **제거 전 placeId 8건을 유지본으로 이전**(일정↔카드 `📄` 링크 보존, `PLACE_TO_CARD`). 보강 3장(6.1 용다리쇼·6.12 메모리스쇼[3.8병합]·1.18 호이안올드타운[placeId 보유라 제거불가]). 9.1 인천 좌표 제거. **레퍼런스(first6) 의존 9곳 완전 제거=독립성**(이미지 5카드 로컬화 + "GitHub 원본" 링크 삭제, 수용기준 `grep -c first6`==0).
- **커스텀 추가 핵심**: "내 일정"에 `＋직접 추가`(제목·날짜·시각 필수/위치·비용 선택). **계획 시점(온라인)에 지오코딩·OSRM 계산→영속 저장→현지 오프라인 재생**(프리셋 ROUTES_CACHE 모델). myPlan 인라인 저장+해시/로컬.
- **원칙(지침)**: 프로젝트는 자기완결적 — 레퍼런스 페이지/객체를 런타임에 참조하지 않음.
- **알려진 잔여 UX 워트(범위 밖, 추후)**: "내 일정 비우기"·"프리셋 선택"이 네이티브 `confirm`/`prompt`(3747·3762행) — 인라인 전환 권장. (참고: reset은 `mp=` 해시를 정상 비움. 공유링크 `#mp=`는 옛 스냅샷을 박제하므로 깨끗한 URL로 열어야 갱신 반영.)
